...
def subseqs(s):
    ...
    if not s: 
        ...
        all_but_first_substr = subseqs(s[1:])
        return all_but_first_substr + insert_into_all(s[0], all_but_first_substr)
...
def make_glookup(class_assignments):
    ...
    "*** YOUR CODE HERE ***"
    cur, tot = 0, 0
    def glookup(subject, score):
        nonlocal cur, tot
        cur += score
        tot += class_assignments[subject]
        return cur/tot
    return glookup
